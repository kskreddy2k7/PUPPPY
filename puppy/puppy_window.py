...
from puppy.state import PuppyState, StateMachine
from puppy.movement import MovementPhysics
from puppy.animation import AnimationController
from puppy.speech_bubble import SpeechBubble
from puppy.personality import PersonalityController, Mood
from puppy.pet_brain import PetBrain
from puppy.achievements import AchievementsManager
from puppy.wellness_engine import WellnessEngine
...
class PuppyWindow(QWidget):
    ...
    def __init__(self, settings_manager, sound_manager, house_window, toy_window, parent=None):
        ...
        # Trigger smart startup greeting based on local computer time
    ...
    def game_loop(self):
        ...
        # If cursor moves while sleeping/going home, automatically wake up and return to cursor
        ...
        # Silent home return ONLY if cursor has not moved for > 2 minutes (120 seconds)
        ...
        if state == PuppyState.SLEEP_WALK:
            ...
            # Cursor movement prediction (predict 2 steps ahead)
    ...
    def anim_loop(self):
        ...
        # Dynamic FPS throttling depending on activity state
    ...
    def open_debug_viewer(self):
        from puppy.debug_viewer import DebugAnimationViewer
        viewer = DebugAnimationViewer(self)
        viewer.exec()
