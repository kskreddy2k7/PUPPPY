...
class MovementPhysics:
    ...
    def update_physics(self, is_running=False, speed_factor=1.0):
        ...
        if dist > 3.0:
            ...
            # Friction deceleration when close to target
            self.vx *= 0.5
            self.vy *= 0.5
            self.ax = 0.0
            self.ay = 0.0
    ...
    def get_speed_category(self, speed: float, cursor_vel: float) -> str:
        """
        Calculates smoothed velocity threshold with hysteresis to eliminate animation flickering.
        Thresholds:
        IDLE < 40
        WALK: 40 - 100
        FAST_WALK: 100 - 180
